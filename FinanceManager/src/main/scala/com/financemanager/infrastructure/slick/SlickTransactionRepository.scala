...
class SlickTransactionRepository(dbManager: SlickDatabaseManager)
  ...
  override def countByCategory(categoryId: CategoryId): Int =
    Await.result(
      db.run(transactions.filter(_.categoryId === categoryId).length.result),
      Duration.Inf
    )

  override def add(input: TransactionInput): Transaction =
    val insertQuery = (transactions.map(t =>
      (t.date, t.amount, t.categoryId, t.description, t.transactionType)
    )
      returning transactions.map(_.id)
      into ((tuple, id) =>
        Transaction(id, tuple._1, tuple._2, tuple._3, tuple._4, tuple._5)
      ))
