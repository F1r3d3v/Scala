...
class SlickDatabaseManager(dbUrl: String):
  private val urlWithFk =
    if (dbUrl.contains("?")) s"$dbUrl&foreign_keys=ON"
    else s"$dbUrl?foreign_keys=ON"
  ...
  def initializeSchema(): Unit =
    ...
    val setupAction = for {
      ...
      _ <-
        if (!tableNames.contains("categories")) categories.schema.create
        else DBIO.successful(())
      _ <-
        if (!tableNames.contains("transactions")) transactions.schema.create
        else DBIO.successful(())
