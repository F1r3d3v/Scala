...
final class ImporterCSV[T](path: Path, hasHeader: Boolean = true)(using decoder: HeaderDecoder[T]) extends Loader[Seq[T]]:
        ...
        if content.isBlank then Left(IOError.EndOfFile(path.toString))
          ...
          content
            ...
            .foldLeft[Either[IOError, List[T]]](Right(Nil)) { (acc, row) =>
              for
                values <- acc
                value <- row.left.map(err => IOError.ReadError(path.toString, err.toString))
              yield value :: values
            }
            .map(_.reverse)
      }.toEither
