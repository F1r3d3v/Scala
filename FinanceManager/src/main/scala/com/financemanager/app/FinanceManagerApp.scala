...
import com.financemanager.infrastructure.slick.{
  SlickDatabaseManager,
  SlickCategoryRepository,
  SlickTransactionRepository
}
...
import com.financemanager.presentation.view.{
  AnalyticsView,
  DashboardView,
  MainView,
  TransactionsView
...
final class FinanceManagerApp extends Application:
  override def start(primaryStage: Stage): Unit =
    ...
    val dbManager = new SlickDatabaseManager(
    ...
    val categoryMaintenanceService =
      new CategoryMaintenanceService(categoryService, repository)
    val importExportService =
      new ImportExportService(repository, transactionService, categoryService)

    val dashboardView = new DashboardView()
    val transactionsView = new TransactionsView()
    val analyticsView = new AnalyticsView()

    val dashboardPresenter =
      DashboardPresenter(dashboardView, budgetService, repository)
    val transactionsPresenter = TransactionsPresenter(
      transactionsView,
      transactionService,
      categoryService,
      categoryMaintenanceService,
    ...
    val analyticsPresenter =
      AnalyticsPresenter(analyticsView, analyticsService, repository)
    ...
    val scene =
      new Scene(mainView.root, AppConfig.WindowWidth, AppConfig.WindowHeight)
