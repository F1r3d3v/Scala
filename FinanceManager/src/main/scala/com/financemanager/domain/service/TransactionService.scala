...
/** Application service responsible for validating and persisting transactions.
  *
  * @param repository
  *   backing repository used for persistence and notifications
  */
final class TransactionService(repository: TransactionRepository):
  ...
  def update(
    ...
    for
      ...
      result <-
        if hasChanged(existing, input) then repository.replace(id, input)
        else Right(existing)
    yield result
  }
  ...
  private[service] def validate(
      input: TransactionInput
  ): Either[DomainError, Unit] =
    if input.amount <= 0 then
      Left(DomainError.InvalidAmount("Amount must be greater than zero"))
    else if input.category.value <= 0 then
      Left(DomainError.InvalidCategory("Category is required"))
    else if input.description.trim.isEmpty then
      Left(DomainError.InvalidDescription("Description is required"))
    else Right(())
