...
import com.financemanager.domain.repository.{
  CategoryRepository,
...
/** Aggregated trend data with labels suitable for charting.
  ...
  * @param granularity
  *   time bucket used when computing the series
  */
final case class TrendResult(
...
/** Domain service for category breakdowns and spend trends.
  ...
  *   source of categories for display names
  */
final class AnalyticsService(
  ...
  def spendingByCategory(
    ...
    transactions.expensesOnly
      ...
      .groupBy(_.category)
      .view
      .mapValues(_.totalAmount)
      .toSeq
      .map { case (catId, amount) =>
        nameById.getOrElse(catId, s"Unknown") -> amount
      }
      .sortBy(-_._2)
  ...
  def spendingTrend(start: LocalDate, end: LocalDate): TrendResult =
    ...
    val formatKey: LocalDate => String = granularity match
      case Granularity.Daily =>
        d => f"${d.getMonthValue}%02d-${d.getDayOfMonth}%02d"
      case Granularity.Weekly =>
        d =>
          f"${d.get(IsoFields.WEEK_BASED_YEAR)}-W${d.get(IsoFields.WEEK_OF_WEEK_BASED_YEAR)}%02d"
      case Granularity.Monthly =>
        d => f"${d.getYear}%04d-${d.getMonthValue}%02d"

    val timeline: Seq[LocalDate] = granularity match
      case Granularity.Daily =>
        (0 to daysBetween).map(start.plusDays(_))
      case Granularity.Weekly =>
        (0 to ChronoUnit.WEEKS.between(start, end).toInt)
          .map(start.plusWeeks(_))
      case Granularity.Monthly =>
        val months = ChronoUnit.MONTHS
          .between(start.withDayOfMonth(1), end.withDayOfMonth(1))
          .toInt
