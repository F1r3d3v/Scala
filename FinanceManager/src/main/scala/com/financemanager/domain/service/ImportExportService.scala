...
final class ImportExportService(
  ...
  def exportTransactions(
      exporter: Writer[Seq[TransactionCsvRow]]
  ): Either[IOError, Unit] =
  ...
  private def buildImportedTransactions(
    ...
    rows
      .foldLeft[Either[IOError, List[TransactionInput]]](Right(Nil)) {
        (acc, row) =>
          acc.flatMap { validInputs =>
            val normalizedCategory = row.category.trim
            if normalizedCategory.isEmpty then
              Left(IOError.ReadError("", "Category is required"))
            else
  ...
  private def buildExportRows: Either[IOError, Seq[TransactionCsvRow]] =
    val categoriesById =
      categoryService.getAll.map(category => category.id -> category.name).toMap

    transactionRepository
      .findAll()
      .foldLeft[Either[IOError, List[TransactionCsvRow]]](Right(Nil)) {
        (acc, transaction) =>
          acc.flatMap { rows =>
            categoriesById
              ...
              .toRight(
                IOError.WriteError(
                  "",
                  s"Category ${transaction.category.value} not found"
                )
              )
              .map { categoryName =>
                TransactionCsvRow(
