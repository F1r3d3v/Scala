...
import com.financemanager.presentation.presenter.{
  AnalyticsPresenter,
  DashboardPresenter,
  TransactionsPresenter
}
import com.financemanager.testutil.{
  MockAnalyticsView,
  MockDashboardView,
  MockTransactionsView,
  TestRepository
}
...
class TransactionsPresenterTest extends FunSuite:
  ...
  test("onViewCreated displays categories and transactions"):
    val repo = TestRepository(
      sampleTransaction(1, BigDecimal("50"), TransactionType.Expense)
    )
    ...
    val categoryMaintenanceService =
      new CategoryMaintenanceService(categoryService, repo)
    ...
    assert(
      view.displayedCategories.nonEmpty,
      clue = "expected categories to be loaded"
    )
    assertEquals(
      view.displayedTransactions.size,
      1,
      clue = "expected one transaction to be displayed"
    )
    assertEquals(
      view.displayedTransactions.head.rawAmount,
      BigDecimal("50"),
      clue = "expected the seeded amount to be shown"
    )
  ...
  test("onSubmit with valid input adds transaction"):
    ...
    assertEquals(
      view.formResets,
      1,
      clue = "expected form to reset after a successful add"
    )
    assertEquals(
      view.displayedTransactions.size,
      1,
      clue = "expected the new transaction to appear"
    )
  ...
  test("onSubmit with invalid input shows error"):
    ...
    assertEquals(
      view.errors.size,
      1,
      clue = "expected a validation error to be shown"
    )
    assertEquals(
      view.formResets,
      0,
      clue = "expected the form to remain unchanged on error"
    )
  ...
  test("onTransactionSelected populates form"):
    ...
    assertEquals(
      view.populatedForms.size,
      1,
      clue = "expected the selected transaction to populate the form"
    )
    assertEquals(
      view.populatedForms.head.id,
      TransactionId(1L),
      clue = "expected the selected id to match"
    )
  ...
  test("onDelete removes transaction and resets form"):
    ...
    assertEquals(
      view.formResets,
      1,
      clue = "expected the form to reset after delete"
    )
    assertEquals(
      view.displayedTransactions.size,
      0,
      clue = "expected the transaction list to be empty after delete"
    )
  ...
  test("onDelete non-existent shows error"):
    ...
    assertEquals(
      view.errors.size,
      1,
      clue = "expected an error for a missing transaction"
    )
  ...
  test("onClearSelection resets form"):
    ...
    assertEquals(
      view.formResets,
      1,
      clue = "expected clear selection to reset the form"
    )
  ...
  test("onSubmit after selection performs update"):
    ...
    assertEquals(
      view.formResets,
      1,
      clue = "expected the form to reset after update"
    )
    assertEquals(
      view.displayedTransactions.head.rawAmount,
      BigDecimal("75"),
      clue = "expected the updated amount to be shown"
    )
  ...
  test("onAddCategory refreshes displayed categories"):
    ...
    assert(
      view.displayedCategories.exists(_.name == "Travel"),
      clue = "expected new category to be displayed"
    )
    assert(
      view.displayedCategoriesHistory.size >= 2,
      clue = "expected categories to refresh after add"
    )
  ...
  test("onDeleteCategory removes category from displayed list"):
    ...
    assert(
      !view.displayedCategories.exists(_.name == "Travel"),
      clue = "expected deleted category to disappear"
    )

  test(
    "deleting category with assigned transactions asks for confirmation and reassigns to Unknown"
  ):
    val repo = TestRepository(
      Transaction(
        TransactionId(1L),
    ...
    val transactionService = TransactionService(repo)
    val categoryService = new CategoryService(
      new InMemoryCategoryRepository(Seq(Category(CategoryId(2L), "Food")))
    )
    val categoryMaintenanceService =
      new CategoryMaintenanceService(categoryService, repo)
    val presenter = TransactionsPresenter(
...
class DashboardPresenterTest extends FunSuite:
  ...
  test("onViewCreated displays summary"):
    ...
    assertEquals(
      ...
      2,
      clue = "expected two transactions in the summary"
    )
    assert(
      summary.totalSpent.contains("100"),
      clue = "expected spent amount to include 100"
    )
    assert(
      summary.totalIncome.contains("500"),
      clue = "expected income amount to include 500"
    )
...
class AnalyticsPresenterTest extends FunSuite:
  ...
  test("onViewCreated displays charts"):
    ...
    assert(
      view.categoryData.nonEmpty,
      clue = "expected category breakdown data"
    )
  ...
  test("onRangeChanged updates data"):
    ...
    assert(
      view.categoryData.nonEmpty,
      clue = "expected category data to remain populated after range change"
    )
  ...
  test("onRangeChanged filters data by range"):
    ...
    presenter.onViewCreated()
    assertEquals(
      view.categoryData.head._2,
      50.0,
      clue = "expected only recent transaction in default 6M range"
    )
    ...
    assertEquals(
      view.categoryData.head._2,
      150.0,
      clue = "expected both transactions in 12M range"
    )

    // Change to custom range excluding the old one
    presenter.onRangeChanged(
      TimeRangeSelection.Custom(now.minusDays(1), now.plusDays(1))
    )
    assertEquals(
      view.categoryData.head._2,
      50.0,
      clue = "expected only recent transaction in custom range"
    )
