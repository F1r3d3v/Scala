...
final class MockTransactionsView extends TransactionsViewContract:
  ...
  var deletionConfirmations: ListBuffer[CategoryDeletionPreview] =
    ListBuffer.empty
  var confirmDeletionResult: Boolean = true

  override def displayTransactions(t: Seq[TransactionDisplay]): Unit =
    displayedTransactions = t
...
final class MockDashboardView extends DashboardViewContract:
  ...
  override def displaySummary(summary: DashboardDisplay): Unit = lastSummary =
    Some(summary)
...
final class MockAnalyticsView extends AnalyticsViewContract:
  ...
  override def displayCategoryBreakdown(data: Seq[(String, Double)]): Unit =
    categoryData = data
  override def displaySpendingTrend(data: TrendData): Unit = trendData = Some(
    data
  )
