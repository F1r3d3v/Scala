...
class JdbcDatabaseManagerTest extends FunSuite:
  ...
  test("ensureCategoriesSeeded should not duplicate categories if called multiple times"):
    withTempDb { manager =>
      manager.initializeSchema()
  ...
  test("ensureCategoriesSeeded should avoid seeding if categories already exist (even custom ones)"):
    withTempDb { manager =>
      manager.initializeSchema()
      Using(manager.getConnection) { conn =>
        Using.resource(conn.prepareStatement("INSERT INTO categories (name) VALUES (?)")) { stmt =>
