...
class SlickDatabaseManagerTest extends FunSuite:
  /** Helper fixture that provisions a temporary SQLite database, instantiates
    ...
    */
  private def withTempDb(
      testCode: (SlickDatabaseManager, String) => Unit
  ): Unit =
  ...
  test(
    "initializeSchema should insert the predefined initial categories when the table is empty"
  ):
    withTempDb { (manager, dbUrl) =>
      manager.initializeSchema()
  ...
  test(
    "initializeSchema should be idempotent and not duplicate categories if called multiple times"
  ):
    withTempDb { (manager, dbUrl) =>
      manager.initializeSchema()
      manager.initializeSchema()
  ...
  test(
    "initializeSchema should avoid seeding if categories already exist (even custom ones)"
  ):
    withTempDb { (manager, dbUrl) =>
      manager.initializeSchema()
      ...
      Using.resource(DriverManager.getConnection(dbUrl)) { conn =>
        Using.resource(conn.createStatement()) { stmt =>
          ...
          stmt.executeUpdate(
            "INSERT INTO categories (name) VALUES ('CustomCategory');"
          )
        }
      }
