...
import com.financemanager.domain.service.{AnalyticsService, Granularity, TrendResult}
...
class AnalyticsServiceTest extends FunSuite:
  ...
  private def expense(amount: BigDecimal, date: LocalDate, categoryName: String = "Food", id: Long = 1) =
    Transaction(TransactionId(id), date, amount, CategoryId(nameToId(categoryName)), "Desc", TransactionType.Expense)

  private def income(amount: BigDecimal, date: LocalDate, categoryId: Long = 1L, id: Long = 1) =
    Transaction(TransactionId(id), date, amount, CategoryId(categoryId), "Desc", TransactionType.Income)
  ...
  test("spendingTrend with short range uses daily granularity"):
    ...
    val result = new AnalyticsService(TestRepository(transactions*), categoryRepo).spendingTrend(start, end)
    assertEquals(result.granularity, Granularity.Daily, clue = "expected daily granularity for short ranges")
  ...
  test("spendingTrend with medium range uses weekly granularity"):
    ...
    val categoryRepo = new InMemoryCategoryRepository(Seq(Category(CategoryId(1L), "Food")))
    val result = new AnalyticsService(TestRepository(transactions*), categoryRepo).spendingTrend(start, end)
    assertEquals(result.granularity, Granularity.Weekly, clue = "expected weekly granularity for medium ranges")
  ...
  test("spendingTrend with long range uses monthly granularity"):
    ...
    val categoryRepo = new InMemoryCategoryRepository(Seq(Category(CategoryId(1L), "Food")))
    val result = new AnalyticsService(TestRepository(transactions*), categoryRepo).spendingTrend(start, end)
    assertEquals(result.granularity, Granularity.Monthly, clue = "expected monthly granularity for long ranges")
    assert(result.points.size >= 8, clue = "expected monthly series to cover the range")
  ...
  test("spendingTrend fills missing periods with zero"):
    ...
    assert(zeroPoints.nonEmpty, clue = "expected missing days to be filled with zero values")

  test("TrendResult periodLabel returns correct strings"):
    assert(TrendResult(Seq.empty, Granularity.Daily).periodLabel == "Daily", clue = "expected daily label")
    assert(TrendResult(Seq.empty, Granularity.Weekly).periodLabel == "Weekly", clue = "expected weekly label")
    assert(TrendResult(Seq.empty, Granularity.Monthly).periodLabel == "Monthly", clue = "expected monthly label")
