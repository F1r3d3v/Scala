...
import com.financemanager.domain.model.{Category, CategoryId, ImportMode, Transaction, TransactionId, TransactionType}
...
class ImportExportServiceTest extends FunSuite:
  ...
  private def buildService(
    ...
    categories: Seq[Category] = Seq(Category(CategoryId(1L), "Food"), Category(CategoryId(4L), "Gift"), Category(CategoryId(2L), "Coffee"), Category(CategoryId(3L), "Salary"))
  ): (ImportExportService, CategoryService) =
    val categoryService = new CategoryService(new InMemoryCategoryRepository(categories))
    (new ImportExportService(repo, TransactionService(repo), categoryService), categoryService)
  ...
  test("import reads CSV rows and persists transactions"):
    val tempFile = Files.createTempFile("transactions", ".csv")
    val input = Seq(
      TransactionCsvRow(LocalDate.parse("2024-02-01"), BigDecimal("15.50"), "Food", "Lunch", TransactionType.Expense),
      TransactionCsvRow(LocalDate.parse("2024-02-05"), BigDecimal("120.00"), "Gift", "Gift", TransactionType.Income)
    )
  ...
  test("import creates missing category based on CSV name"):
    val tempFile = Files.createTempFile("transactions", ".csv")
    val input = Seq(
      TransactionCsvRow(LocalDate.parse("2024-02-01"), BigDecimal("15.50"), "New Category", "Lunch", TransactionType.Expense)
    )
    ...
    val repo = TestRepository()
    val (service, categoryService) = buildService(repo, Seq(Category(CategoryId(1L), "Food")))
    val importer = ImporterCSV(tempFile)

    val result = service.importTransactions(importer, ImportMode.Append)

    assert(result.isRight, clue = "expected import with new category to succeed")
    assertEquals(categoryService.findByName("New Category").map(_.name), Some("New Category"))
    assertEquals(repo.findAll().head.description, "Lunch")
